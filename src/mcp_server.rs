...
impl McpServer {
    pub async fn new() -> Result<Self> {
        ...
        let known_hosts = Arc::new(Mutex::new(
            Self::load_known_hosts(&config.known_hosts_path).await,
        ));

        // Load saved configurations
        let saved_configs = Arc::new(Mutex::new(
            Self::load_saved_configs(&config.saved_configs_path).await,
        ));

        // Initialize audit logger if enabled
        let audit_logger = if config.enable_audit_logging {
            Some(Arc::new(Mutex::new(AuditLogger::new(
            ...
            Box::pin(async move {
                ...
                if let Some(logger) = &audit_logger {
                    logger
                        .lock()
                        .await
                        .log_tool_call(&params.name, &params.arguments)
                        .await;
                }
                ...
                let result = match params.name.as_str() {
                    ...
                    "ssh_list_sessions" => tools::ssh_list_sessions(session_manager.clone()).await,
                    "ssh_upload_file" => {
                    ...
                    "ssh_manage_keys" => tools::ssh_manage_keys(params.arguments).await,
                    "ssh_verify_host" => {
                    ...
                    "ssh_credential_store" => {
                        tools::ssh_credential_store(params.arguments, credential_provider.clone())
                            .await
                    }
                    _ => Err(SshMcpError::McpProtocol(format!(
                        "Unknown tool: {}",
                        params.name
                    ))),
                };
        ...
        io.add_method("prompts/get", move |params: Params| {
            ...
            Box::pin(async move {
                ...
                match params.name.as_str() {
                    "ssh_config" => prompts::get_ssh_config_prompt(params.arguments).await,
                    "ssh_quick_connect" => {
                        prompts::get_quick_connect_prompt(params.arguments, saved_configs.clone())
                            .await
                    }
                    _ => Err(SshMcpError::McpProtocol(format!(
                        "Unknown prompt: {}",
