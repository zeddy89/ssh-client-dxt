...
impl SshClient {
    pub fn connect(config: &SshConfig) -> Result<Session> {
        ...
        let host_key = session.host_key().ok_or_else(|| {
            SshMcpError::HostVerificationFailed("No host key available".to_string())
        })?;
    ...
    fn authenticate(session: &mut Session, config: &SshConfig) -> Result<()> {
        ...
        if let Some(key_path) = &config.private_key_path {
            ...
            let result = if let Some(passphrase) = &config.passphrase {
                session.userauth_pubkey_file(&config.username, None, key_path, Some(passphrase))
            } else {
        ...
        if let Some(password) = &config.password {
            ...
            session
                .userauth_keyboard_interactive(&config.username, &mut prompt)
                .map_err(|e| SshMcpError::AuthenticationFailed(e.to_string()))?;
        }
    ...
    pub fn upload_file(session: &Session, local_path: &Path, remote_path: &str) -> Result<()> {
        ...
        let sftp = session.sftp().map_err(|e| {
            SshMcpError::FileOperation(format!("SFTP initialization failed: {}", e))
        })?;
        ...
        let mut remote_file = sftp.create(Path::new(remote_path)).map_err(|e| {
            SshMcpError::FileOperation(format!("Failed to create remote file: {}", e))
        })?;
    ...
    pub fn download_file(session: &Session, remote_path: &str, local_path: &Path) -> Result<()> {
        ...
        let mut remote_file = sftp.open(Path::new(remote_path)).map_err(|e| {
            SshMcpError::FileOperation(format!("Failed to open remote file: {}", e))
        })?;

        let mut local_file = std::fs::File::create(local_path).map_err(|e| {
            SshMcpError::FileOperation(format!("Failed to create local file: {}", e))
        })?;
