use crate::config::{SshConfig, PortForwardConfig, PortForwardType};
        ...
        if let Some(host_key) = session.host_key() {
            ...
            if let Some(known_fp) = hosts.get(&format!("{}:{}", config.host, config.port)) {
                if known_fp != &fingerprint {
                    return Err(SshMcpError::HostVerificationFailed(
                        format!("Host key mismatch for {}:{}", config.host, config.port)
                    ));
                }
            }
        }
...
pub async fn ssh_execute(
    ...
    let result = session_manager.with_session(&params.session_id, |session| {
        let (stdout, stderr, exit_code) = SshClient::execute_command(&session.session, &params.command)?;
        Ok(json!({
            "stdout": stdout,
            "stderr": stderr,
            "exitCode": exit_code,
...
pub async fn ssh_disconnect(
    ...
    let params: DisconnectParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
...
pub async fn ssh_list_sessions(
    ...
    let session_list: Vec<Value> = sessions.into_iter().map(|(id, config, created_at)| {
        json!({
            "sessionId": id,
            "host": config.host,
            "port": config.port,
...
pub async fn ssh_upload_file(
    ...
    let params: FileTransferParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
    
    session_manager.with_session(&params.session_id, |session| {
...
pub async fn ssh_port_forward(
    ...
    let params: PortForwardParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
...
pub async fn ssh_manage_keys(params: Value) -> Result<Value> {
    let params: KeyManageParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
...
pub async fn ssh_verify_host(
    ...
    let params: HostVerifyParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
    ...
    match params.action.as_deref().unwrap_or("verify") {
        ...
        _ => Err(SshMcpError::Validation(format!("Invalid action: {}", params.action.unwrap())))
    }
}
...
pub async fn ssh_config_manage(
    ...
    let params: ConfigManageParams = serde_json::from_value(params)
        .map_err(|e| SshMcpError::Validation(e.to_string()))?;
    
    match params.action.as_str() {
        "list" => {
            let configs = saved_configs.lock().await;
            let config_list: Vec<Value> = configs.iter().map(|(name, config)| {
                json!({
                    "name": name,
                    "host": config.host,
                    "port": config.port,
        ...
        "load" => {
            ...
            if let Some(config) = configs.get(&name) {
                ...
                Err(SshMcpError::Configuration(format!("Config not found: {}", name)))
            }
        },
        "delete" => {
            let name = params.name.ok_or_else(|| 
